...
export async function $(literals: TemplateStringsArray, ...values: any[]) {
  ...
  try {
    ...
    if (error.stdout)
      error.stdout = 'value removed by nuxt/ecosystem-ci'
    if (error.stderr)
      error.stderr = 'value removed by nuxt/ecosystem-ci'
    if (error.stdio)
      error.stdio = ['value removed by nuxt/ecosystem-ci']
    throw error
  }
...
export async function setupEnvironment(): Promise<EnvironmentData> {
  ...
  return { root, workspace, nuxtPath, cwd, env }
}
...
export async function runInRepo(options: RunOptions & RepoOptions) {
  ...
  if (overrides['@nuxtjs/i18n'] !== false) {
    overrides['@nuxtjs/i18n'] ||= 'latest'
  }
  const vueResolution
